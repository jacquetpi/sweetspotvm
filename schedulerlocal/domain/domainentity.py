...
class DomainEntity:
    ...
    def set_cpu_pin(self, resource_id : int, template_pin : tuple):
        """Set CPU pin situation based on template
        ----------
        """
        if self.cpu_pin == None:
            self.cpu_pin = list()
        for _ in range((resource_id+1) - len(self.cpu_pin)): self.cpu_pin.append(list()) # initialization
        self.cpu_pin[resource_id] = template_pin
    ...
    def __str__(self):
        ...
        ----------
        """
        return 'vm ' + self.get_name() + ' ' + str(self.get_cpu()) + 'vCPU ' + str(self.get_mem(as_kb=False)) + 'MB\n'